...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 21 16:08:15 2021
...
def save_plot(examples, epoch, n=10):
	...
	for i in range(n * n):
		...
		pyplot.axis('off')
		# plot raw pixel data
		pyplot.imshow(examples[i, :, :, 0], cmap='magma')
	# save plot to file
	filename = '0_generated_plot_e%03d.png' % (epoch+1)
...
for i in range(1):
    ...
    # generate images
    n_images = 100
    latent_points = generate_latent_points(100, n_images)
    # generate images
    X = model.predict(latent_points)
    np.save('fake_%d' % i, X)
    
    save_plot(X, int(np.sqrt(n_images)))

...
for repeat, fold, par_idxs, val_idxs in tqdm(outerfold.split(X, y, groups), total=n_repeats*n_outer):
    ...
    Xunder, yunder = RandomUnderSampler(sampling_strategy={np.argmax(counts) : counts[np.argsort(counts)[-2]]}).fit_resample(Xpar, ypar)
    Xover, yover = SMOTE(sampling_strategy="not majority", n_jobs=1).fit_resample(Xunder, yunder)

    ytemp = np.empty((5, len(yval)))
    ...
    for i in range(5):
        ...
        ytrain[yover == i] = 1
        ytrain[yover != i] = 0
        model.fit(Xover, ytrain)
        ytemp[i] = model.predict(Xval)
    ytemp = ytemp.T

    for i in range(len(yval)):
        if np.sum(ytemp[i]) == 1:
            ypred[repeat][val_idxs[i]] = np.where(ytemp[i] == 1)[0]
        else:
            ypred[repeat][val_idxs[i]] = 5

...
for fold, (par_idxs, val_idxs) in enumerate(outerfold.split(X, y, groups)):
    if verbose: pbar.set_description("#{} repeat #{} fold".format(fold % n_outer, fold // n_outer))
...
prediction_name = "predictions_{}_{}_{}_{}_{}_{}_{}".format(modelname, technic, n_repeats, n_outer, n_inner, seed, now) 
np.save(prediction_name, ypred, allow_pickle=True)
hyperparameter_name = "hyperparameters_{}_{}_{}_{}_{}_{}_{}".format(modelname, technic, n_repeats, n_outer, n_inner, seed, now)
with open(hyperparameter_name, 'w') as file:
    p = {param.name : value for param, value in zip(hyperspace, model_opt.x)}
    dump(p, file)
#%%

...
if False:
    ...
    parser.add_argument('--n_jobs_optimize', action='store', type=int, required=False, default=1)
    ...
    n_jobs_optimize = args.n_jobs_optimize
...
samplers = {
    'RandomUnder': {'sampler': ('under', RandomUnderSampler()),
                    'space': []},
    'NearMiss': {'sampler': ('under', NearMiss(version=3, n_jobs=1)),
                 'space': []},
...
if 'under' in list(zip(*steps))[0]:
    hyperspace.append(Real(0.01, 0.05, name='special__undersamplingratio'))
...
def train_and_eval(repeat, fold, par_idxs, val_idxs):
    ...
    def objective(**params):
        ytemp = np.empty(len(ypar), dtype=int)
        ytemp.fill(-1)
        
        for i, (train_idxs, test_idxs) in enumerate(innerfold.split(Xpar, ypar)):
            params = format_params(params, ypar[train_idxs])
            pipe.set_params(**params)

            try:
                pipe.fit(Xpar[train_idxs], ypar[train_idxs])
                ytemp[test_idxs] = pipe.predict(Xpar[test_idxs])

            except Exception as error:
                warnings.warn("Pipeline error: " + str(error))

        return fbeta_score(ypar[ytemp != -1], ytemp[ytemp != -1], beta=2, average="weighted")
...
out = Parallel(n_jobs=n_parallel, pre_dispatch="{}*n_jobs".format(n_jobs_optimize), verbose=0)(

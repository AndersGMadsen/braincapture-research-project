...
if False:
    ...
    parser.add_argument('--downsample', action='store', type=float, required=False, default=-1.0)
    ...
    modelname = "LDA"
    technique = 9
    outdir = r"C:\Users\andersgm\Documents\Courses\02466 Project work F21\Project\epilepsy-project\SamplingExperiment\Results"
    optimize = 10
    logging = r"C:\Users\andersgm\Documents\Courses\02466 Project work F21\Project\epilepsy-project\SamplingExperiment\Results\logging.log"
    downsample = 1
...
def strategy(y):
    counts = [np.sum(y == l) for l in range(6)]
    return {np.argmax(counts) : counts[np.argsort(counts)[-2]]}

samplers = {  
    'RandomUnder':  {'sampler': ('under', RandomUnderSampler(sampling_strategy=strategy)),
                     'space': []},
    'NearMissv1':   {'sampler': ('under', NearMiss(version=1, n_jobs=1, sampling_strategy=strategy)),
                     'space': []},
    'NearMissv2':   {'sampler': ('under', NearMiss(version=2, n_jobs=1, sampling_strategy=strategy)),
                     'space': []},
    'NearMissv3':   {'sampler': ('under', NearMiss(version=3, n_jobs=1, sampling_strategy=strategy)),
                     'space': []},
    'RandomOver':   {'sampler': ('over', RandomOverSampler(sampling_strategy='not majority')),
                     'space': []},
    'SMOTE':        {'sampler': ('over', SMOTE(n_jobs=1, sampling_strategy='not majority')),
                     'space': []},
}
...
techniques = [
    ...
    [samplers['NearMissv1'], samplers['RandomOver']],
    [samplers['NearMissv2'], samplers['RandomOver']],
    [samplers['NearMissv3'], samplers['RandomOver']],
    [samplers['RandomUnder'], samplers['SMOTE']],
    [samplers['NearMissv1'], samplers['SMOTE']],
    [samplers['NearMissv2'], samplers['SMOTE']],
    [samplers['NearMissv3'], samplers['SMOTE']]
]
    ...
    if verbose:
        ...
        with open(logging, "a") as output:
            output.write("[repeat: {}, fold: {}] Started at {}\n".format(repeat, fold,
                                                                         time.asctime(time.localtime(start))[11:19]))
...
out = Parallel(n_jobs=n_parallel, pre_dispatch="1*n_jobs", verbose=0)(
...
prediction_name = 'predictions_{}_{}_{}_{}_{}_{}_{}'.format(modelname, technique, n_repeats, n_outer, n_inner, seed,
                                                            now)
np.save(outdir + '/' + prediction_name, ypred, allow_pickle=True)
hyperparameter_name = 'hyperparameters_{}_{}_{}_{}_{}_{}_{}'.format(modelname, technique, n_repeats, n_outer, n_inner,
                                                                    seed, now)

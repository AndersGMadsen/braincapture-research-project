...
if 'under' in list(zip(*steps))[0]:
    hyperspace.append(Real(0.03, 0.04, name='special__undersamplingratio'))
...
pbar = tqdm(total=n_outer*n_inner*n_repeats*optimize)
...
for repeat, fold, par_idxs, val_idxs in tqdm(outerfold.split(X, y, groups)):
    ...
    model_opt = forest_minimize(objective, hyperspace, n_calls=optimize, n_jobs=5)
    ...
    ypred[repeat][val_idxs] = prediction
